document.querySelector("#btn")
      .addEventListener("click", function quote() {
        generate();
      });
    ...
    generate = () => {
      ...
      var authors = Object.keys(quotes);//sare author ke nam authors naam variable mein as an array store hogye
      // grab a random key (author) and store it in author
      var author = authors[Math.floor(Math.random() * authors.length)];//randomly author ka naam aayega
      // grab the value(quote) that belongs to that key
      var quote = quotes[author];

      document.querySelector("#box1").textContent = quote;
      document.querySelector("#box2").textContent = author;
    }